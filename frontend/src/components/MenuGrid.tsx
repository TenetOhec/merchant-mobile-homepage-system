import { useEffect, useState, type CSSProperties } from 'react';
import { getIcon } from '../lib/icons';
import type { MenuItem } from '../types/config';
import { sortByVisibleAndSort } from '../lib/helpers';

const menuAssetMap: Record<string, string> = {
  商品管理: '/extracted_icons/01_商品管理.png',
  营销工具: '/extracted_icons/02_营销工具.png',
  数据中心: '/extracted_icons/03_数据中心.png',
  推广平台: '/extracted_icons/05_推广平台.png',
  打单工具: '/extracted_icons/06_打单工具.png',
  质量体验: '/extracted_icons/07_质量体验.png',
  品质诊断: '/extracted_icons/08_品质诊断.png',
  防控中心: '/extracted_icons/09_防控中心.png',
  更多: '/extracted_icons/10_更多.png'
};

const marketingActivityImages = [
  '/extracted_icons/04_营销活动4.png',
  '/extracted_icons/04_营销活动5.png',
  '/extracted_icons/04-营销活动3.png'
];

const HOLD_MS = 1800;
const FADE_MS = 420;
const FLIP_MS = 420;
const FLIP_PAUSE_MS = 180;
const PULSE_DELAY_MS = 220;
const SECOND_SEQUENCE_MS = 1440;
const ICON_CLASS_NAME = 'h-[40px] w-[40px] object-contain';

function getMarketingImageWrapperClassName(index: number) {
  if (index === 0 || index === 1) {
    return 'absolute inset-0 flex items-center justify-center scale-[0.8]';
  }

  return 'absolute inset-0 flex items-center justify-center';
}

function getIdleImageStyle(index: number, transition: 'idle' | 'fade' | 'flip', pulseEnabled: boolean): CSSProperties | undefined {
  if (transition === 'flip') {
    return {
      animation: `marketingFlipOut ${FLIP_MS}ms cubic-bezier(0.32, 0.72, 0.2, 1) forwards`,
      transformStyle: 'preserve-3d'
    };
  }

  if (index === 0 && pulseEnabled) {
    return {
      animation: 'marketingPulse 1.45s ease-in-out infinite'
    };
  }

  if (index === 1) {
    return {
      animation: `marketingSecondSequence ${SECOND_SEQUENCE_MS}ms cubic-bezier(0.22, 0.7, 0.2, 1) forwards`
    };
  }

  return undefined;
}

function getTransitionOutStyle(fromIndex: number, toIndex: number): CSSProperties {
  if (fromIndex === 1 && toIndex === 2) {
    return {
      animation: `marketingSecondFadeOut ${FADE_MS}ms linear forwards`
    };
  }

  return {
    animation: `marketingFirstFadeOut ${FADE_MS}ms ease forwards`
  };
}

function getTransitionInStyle(toIndex: number): CSSProperties | undefined {
  if (toIndex === 1) {
    return {
      animation: `marketingSecondEnter ${FADE_MS}ms linear forwards`
    };
  }

  return undefined;
}

function MarketingActivityIcon() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [nextIndex, setNextIndex] = useState<number | null>(null);
  const [transition, setTransition] = useState<'idle' | 'fade' | 'flip'>('idle');
  const [pulseEnabled, setPulseEnabled] = useState(true);

  useEffect(() => {
    let timer = 0;

    if (transition === 'idle') {
      const holdMs = currentIndex === 1 ? SECOND_SEQUENCE_MS : HOLD_MS;
      timer = window.setTimeout(() => {
        if (currentIndex === 0) {
          setNextIndex(1);
          setTransition('fade');
          return;
        }

        if (currentIndex === 1) {
          setCurrentIndex(2);
          setPulseEnabled(false);
          setTransition('idle');
          return;
        }

        setTransition('flip');
      }, holdMs);
    }

    if (transition === 'fade' && nextIndex !== null) {
      timer = window.setTimeout(() => {
        setCurrentIndex(nextIndex);
        setNextIndex(null);
        setPulseEnabled(false);
        setTransition('idle');
      }, FADE_MS);
    }

    if (transition === 'flip') {
      timer = window.setTimeout(() => {
        setCurrentIndex(0);
        setPulseEnabled(false);
        setTransition('idle');
      }, FLIP_MS + FLIP_PAUSE_MS);
    }

    return () => {
      window.clearTimeout(timer);
    };
  }, [currentIndex, nextIndex, transition]);

  useEffect(() => {
    if (transition !== 'idle' || pulseEnabled) {
      return;
    }

    const timer = window.setTimeout(() => {
      setPulseEnabled(true);
    }, PULSE_DELAY_MS);

    return () => {
      window.clearTimeout(timer);
    };
  }, [pulseEnabled, transition]);

  if (transition === 'fade' && nextIndex !== null) {
    return (
      <div className="relative h-[40px] w-[40px]">
        <div className={getMarketingImageWrapperClassName(nextIndex)}>
          <img
            src={marketingActivityImages[nextIndex]}
            alt="营销活动"
            className={ICON_CLASS_NAME}
            style={getTransitionInStyle(nextIndex)}
          />
        </div>
        <div className={getMarketingImageWrapperClassName(currentIndex)}>
          <img
            src={marketingActivityImages[currentIndex]}
            alt="营销活动"
            className={ICON_CLASS_NAME}
            style={getTransitionOutStyle(currentIndex, nextIndex)}
          />
        </div>
      </div>
    );
  }

  return (
    <div className="relative h-[40px] w-[40px] overflow-hidden">
      <div className={getMarketingImageWrapperClassName(currentIndex)}>
        <img
          src={marketingActivityImages[currentIndex]}
          alt="营销活动"
          className={ICON_CLASS_NAME}
          style={getIdleImageStyle(currentIndex, transition, pulseEnabled)}
        />
      </div>
    </div>
  );
}

export function MenuGrid({ menus }: { menus: MenuItem[] }) {
  const visibleMenus = sortByVisibleAndSort(menus).slice(0, 10);

  return (
    <section className="mx-[6px] mt-[3px] rounded-[8px] bg-white px-[4px] pb-[10px] pt-[4px] shadow-[0_10px_24px_rgba(35,39,49,0.06)]">
      <style>{`
        @keyframes marketingPulse {
          0% { transform: scale(0.95); }
          50% { transform: scale(1.03); }
          100% { transform: scale(0.95); }
        }
        @keyframes marketingSecondEnter {
          from { opacity: 0; transform: scale(1.3); }
          to { opacity: 1; transform: scale(1.3); }
        }
        @keyframes marketingSecondFadeOut {
          from { opacity: 1; }
          to { opacity: 0; }
        }
        @keyframes marketingFirstFadeOut {
          from { opacity: 1; transform: scale(0.98); }
          to { opacity: 0; transform: scale(1.05); }
        }
        @keyframes marketingSecondSequence {
          0% { opacity: 1; transform: scale(1.3); }
          42% { opacity: 1; transform: scale(1); }
          100% { opacity: 1; transform: scale(1); }
        }
        @keyframes marketingFlipOut {
          0% { opacity: 1; transform: rotateY(0deg) scale(0.96); }
          35% { opacity: 1; transform: rotateY(80deg) scale(1.03); }
          75% { opacity: 1; transform: rotateY(180deg) scale(0.97); }
          100% { opacity: 1; transform: rotateY(180deg) scale(0.97); }
        }
      `}</style>
      <div className="grid grid-cols-5 gap-y-[8px]">
        {visibleMenus.map((item) => {
          const assetIcon = menuAssetMap[item.title];
          const FallbackIcon = getIcon(item.icon);
          return (
            <div key={item.title} className="relative flex flex-col items-center gap-[1px] px-[1px]">
              <div className="relative flex h-[42px] w-[42px] items-center justify-center">
                {item.title === '营销活动' ? (
                  <MarketingActivityIcon />
                ) : assetIcon ? (
                  <img src={assetIcon} alt={item.title} className="h-[36px] w-[36px] object-contain" />
                ) : (
                  <FallbackIcon size={28} strokeWidth={1.9} className="text-[#5a5e68]" />
                )}
              </div>
              <div className="text-center text-[14px] leading-[1.08] text-[#2f3137]">{item.title}</div>
            </div>
          );
        })}
      </div>
    </section>
  );
}
